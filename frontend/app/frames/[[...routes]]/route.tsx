...
import {NEXT_PUBLIC_URL, fixUrls} from "@/src/urls";
...
app.frame("/frog/:id", async (c) => {
  ...
  const checkStatus = <Button value="refresh" action={`/frog/${frog?.id}`}>Check Status</Button>
  const refresh = <Button value="refresh" action={`/frog/${frog?.id}`}>Refresh</Button>
  const {buttonValue, inputText, status} = c
  let intents = [<Button.Reset>Back</Button.Reset>]
  if (frog) {
    if (frog.alive) {
      if (visitor?.actionsAllowed) {
        intents = [
          <Button value="feed" action={`/frog/${frog.id}`}>🥗 Feed</Button>,
          <Button value="play" action={`/frog/${frog.id}`}>🎮 Play</Button>,
          <Button value="hug" action={`/frog/${frog.id}`}>💗 Hug</Button>,
          refresh
        ]
      } else {
        intents = [checkStatus]
      }
    } else if (visitor?.mintParameters) {
      intents = [
  ...
  return c.res({
    image: (frog?.alive ?
        <FrogProfileFrame frog={frog} hideStats={!buttonValue}>
        ...
        : frog ? <FrogProfileFrame frog={frog} hideStats={true}>
