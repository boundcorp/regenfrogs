...
def _do_validation(info, input_tree, input_arg, **kwargs):
    ...
    root_validator = _unwrap_validator(getattr(input_arg, "get_type", lambda: input_arg.type)())
    ...
    fields_to_validate, subtrees_to_validate = _unpack_input_tree(input_tree, root_validator)
    ...
    for ftv in fields_to_validate:
        ...
        try:
            ...
            if new_value != value:
                ...
                field = functools.reduce(lambda obj, k: obj[k] if k else obj, path[:-1], input_tree)
